...
| P1 | 回测证据完整性（脚本解析真实数字，而非 agent 自述；**含日期缺口/schema 前置校验 + 前瞻偏差位移重放测试**，见 §3） | 全部下游判断建立在假数据上 |
| P2 | 长期防漂移/审计（仅 `data_hash`——不影响"这次数字对不对"，只影响"未来能否复现"） | 数字会随时间被静默污染 |
| P3 | 多项目分发与配置层 | 效率/维护成本，不影响正确性 |
| P4 | 代码/流程卫生（LOC 预算、PARTIAL 记录、prompt 顺序） | 返工成本，不影响正确性或安全 |

> **排序轴的澄清**（spotcat-e4 两轮复核指出的同一类分歧）：P0-P4 是"出错代价"排序，不是严格的"先跑完这层
> 才跑下一层"执行顺序，也不是"谁决定这次数字可信度"的判定轴。日期缺口校验、前瞻偏差位移重放测试都属于**决
> 定当次数字是否可信**——数据有缺口或信号用了未来数据，这一次算出来的 Sharpe 从一开始就是假的——因此两者
> 都是 P1 gate-runner 流程内的门控项，任一不过 = P1 直接 FAIL，不是"放进 P2 跟 P1 并行、不阻塞"。P2 只剩
> `data_hash` 一条：数据被静默改过不代表这一次的数字是假的，只是以后没法验证复现，这条才是真正跟 P1 执行
> 独立、不影响当次判定的审计项。

```
P0 → P1（含日期缺口前置校验 + 前瞻偏差位移重放测试）→ P2（先在一个试点项目里跑通全部，完成至少一次真实 sprint 的 canary 验证）
...
缓解。`quality-reviewer-prompt.md` 读的是 harness 落盘的这份文件，不是 agent 自己转述的内容。落盘文件必须
带 `run_id`，`quality-reviewer-prompt.md` 只认与**当前** run_id 匹配的文件——防止 agent 引用上一轮的旧结
果冒充这一轮的（spotcat-e4 补充建议）。
...
**前瞻偏差（位移重放测试）也是 P1 的一部分，不是 P2。**（spotcat-e4 第二轮复核发现，跟日期缺口是同一类
分类错误）信号函数分别喂 `data[:T]` 和 `data[:T+k]`，断言 T 处输出逐位相同，抓住"代码里直接用了未来数据"
的大多数真实 bug；抓不住"数据在 T 时刻是否真的已披露"这类时点可得性问题（重放测试本身是确定性的），这部
分留给 LLM 读代码。**初版把它错放进了 P2**——但重放测试失败意味着信号函数用了未来数据，**这一次**算出来
的 Sharpe/回撤本身就是假的，跟"日期缺口""解析失败"是同一严重程度，不是"以后可能会漂移"的审计问题。schema
里的 `lookahead_check` 字段失败 = P1 直接 FAIL，不是"P2 阶段跟 P1 并行验证、不阻塞"。§9 验收标准与流程图
据此调整（见下）。

## 4. P2：长期防漂移/审计（不是执行阶段，是跟 P1 并列的独立检查，不影响当次数字可信度）

P2 只保留一条真正独立于"这一次数字对不对"、只影响"未来能不能复现/会不会被污染"的检查：

**历史数据不可变性：`data_hash`。** schema 里带 `data_hash` 字段，脚本对本次用到的数据文件区间算 hash 写
进结果——不是为了当次门控用，是为了未来审计：某天回测结果对不上，第一件要排除的事是"数据源是不是被静默
改过"（数据商重新调整了某天价格、修 bug、静默补缺失数据），没有 hash 记录这个问题永远无法确认。数据被静
默改过不代表这一次算出来的数字是假的，只是以后没法验证复现——这是它和前瞻偏差检测的本质区别，也是它留在
P2 而不是并入 P1 的原因。
...
- [ ] P1：`gate-runner.py` 单次运行输出全部 gate 的统一 JSON，且 JSON 由 Cron harness 独立落盘捕获、带
      `run_id`（不经 agent 转述，且 quality-reviewer 只认当前 run_id 的文件）；`backtest-result.schema.json`
      定稿，至少一个真实项目写出 adapter 并跑通；解析失败与阈值未达标为两种不同状态；**日期缺口/schema 前
      置校验、前瞻偏差位移重放测试均跑在指标解析流程内，任一不过 = P1 直接 FAIL，不放行**
- [ ] P2：`data_hash` 落地（唯一的审计/长期防漂移项，跟 P1 并行验证，不阻塞 P1 判定）
